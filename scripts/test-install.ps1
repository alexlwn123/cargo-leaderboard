...
try {
    ...
    $script:mirror = Join-Path $root 'mirror'
    New-Item -ItemType Directory $script:mirror | Out-Null
    $name = Split-Path $archivePath -Leaf
    Copy-Item $archivePath (Join-Path $script:mirror $name)
    $sumPath = Join-Path $script:mirror 'SHA256SUMS'
    ...
    function Invoke-WebRequest {
        ...
        Copy-Item (Join-Path $script:mirror ($Uri.Split('/')[-1])) $OutFile
    }
    $bin = Join-Path $root 'bin with spaces'
    $env:CARGO_LEADERBOARD_CONFIG_DIR = Join-Path $root 'config'
    & ./public/install.ps1 -Version v0.2.0 -BinDir $bin
    ...
    $rejected = $false
    try { & ./public/install.ps1 -Version v0.2.0 -BinDir $bin } catch { $rejected = $true }
