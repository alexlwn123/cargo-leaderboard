...
export async function leaderboard(metric, limit, sql = database()) {
  ...
  const entries = await sql.query(
    ...
    WITH ranked AS (
      ...
      FROM build_events WHERE github_id IS NOT NULL AND command = $1 AND success AND ${column} > 0
    ) SELECT event_id, github_users.github_id::text, github_login, repo_slug, bytes, file_count, duration_ms, profile, platform, finished_at
...
export async function submit(event, user, sql = database()) {
  ...
  await sql`
    INSERT INTO build_events (event_id, nickname, github_id, repo_slug, command, started_at, finished_at, duration_ms, success, cargo_version, client_version, bytes, bytes_before, bytes_after, file_count, profile, platform)
    VALUES (${event.event_id}, ${user.github_login}, ${user.github_id}, ${event.repo_slug}, ${event.command}, ${event.started_at}, ${event.finished_at}, ${event.duration_ms}, ${event.success}, ${event.cargo_version}, ${event.client_version}, ${event.bytes}, ${event.bytes_before}, ${event.bytes_after}, ${event.file_count}, ${event.profile}, ${event.platform})
